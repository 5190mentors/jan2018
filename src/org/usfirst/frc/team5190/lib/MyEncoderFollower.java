...
public class MyEncoderFollower {
    ...
    double last_error = 0, heading = 0;
    boolean bufferedValueSet = false;		// have we already computed the value for this segment
     ...
     */
    public void reset() {
        last_error = 0; segment = 0; bufferedValueSet = false; bufferedValue = 0;
    }
    ...
    public double calculate(int encoder_tick) {
        ...
        if (segment < trajectory.length()) {
            ...
            if (error < 0) {
            	// we are done with this segment - go to the next segment
            	segment++;
            	bufferedValueSet = false;
            	return calculate(encoder_tick);
            }
            
            if (bufferedValueSet) {
            	// we already computed the value for this segment.
            	return bufferedValue;
            }
            
            // we have not yet computed the value - so calculate and buffer
...
//            
            ...
            double calculated_value = kv * seg.velocity;
            
            last_error = error;
            heading = seg.heading;
            bufferedValue = calculated_value;
            bufferedValueSet = true;
            
            return calculated_value;
        } else {
        	// we are done with all segments
        	return 0;
        }
    }
